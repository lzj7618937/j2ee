function _addCookie(b, f, d, e) {
    var c = "";
    if (e) {
        c = b + "=" + escape(f) + "; Path=/;domain=" + e + ";"
    } else {
        c = b + "=" + escape(f) + "; Path=/;"
    }
    if (d > 0) {
        var a = new Date();
        a.setTime(a.getTime() + d * 3600000);
...
function _getCookie(a) {
    ...
    if (document.cookie.match(c)) {
        var b = document.cookie.match(c)[0];
        var e = b.indexOf("=");
        if (e > 0) {
            return b.substring(e + 1).replace(/^\s+|\s+$/g, "")
        }
    } else {
        return ""
    }
...
if (_getCookie("ca_sessionid") == "") {
    var newuid = _makeUID();
    _addCookie("ca_sessionid", newuid, 0, _parentDm)
}
...
function _caTraker() {
    ...
    this._init = function (domain, tenantID, ptype) {
        ...
        if (ca_ptype === "L") {
            ...
            if (ptype != null && ptype.length == 1 && (ptype === "N" || ptype === "L" || ptype === "C" || ptype === "T")) {
                ca_ptype = ptype
            } else {
                ca_ptype = "N"
            }
    ...
    this._setTouch = function (t) {
        ...
        if (ca_se === "") {
            ...
            if (tmp_se.indexOf(ca_domain) < 0) {
                ca_ptype = "L";
                if (ca_touch === "L") {
                    _addCookie("ca_se", tmp_se, 36 * 30 * 24, _parentDm)
                } else {
                    if (ca_touch === "S") {
                        if (tmp_se.search(/.baidu./i) >= 0 || tmp_se.search(/.google./i) >= 0 || tmp_se.search(/.sogou./i) >= 0 || tmp_se.search(/.bing./i) >= 0 || tmp_se.search(/.yahoo./i) >= 0 || tmp_se.search(/.woso./i) >= 0 || tmp_se.search(/.soso./i) >= 0 || tmp_se.search(/.youdao./i) >= 0) {
        ...
        if (ca_trace === "") {
            ...
            for (var i = 0; i < ca_params.length; i++) {
                if (ca_params[i].search(/ca_source/) >= 0 || ca_params[i].search(/utm_source/) >= 0) {
                    ...
                    if (ca_params[i].search(/ca_keyword/) >= 0 || ca_params[i].search(/utm_term/) >= 0) {
                        ca_params[i] = ca_source
                    }
                }
            }
    ...
    this._checkTraceCode = function () {
        ...
        if (location.search !== "") {
            ...
            for (var i = 0; i < params.length; i++) {
                if (params[i].search(/^ca_/i) >= 0 || params[i].search(/^utm_/i) >= 0) {
                    ca_trace += params[i] + "&"
                }
            }
    ...
    this._monitorEvent = function (tagType, propName, propValue, ptype, action) {
        var jsonStr = '{"name":"' + propName + '","value":"' + propValue + '","ptype":"' + (ptype == null ? "" : ptype) + '","action":"' + (action == null ? "" : action) + '"}';
    ...
    this._tracePage = function (act) {
        ...
        query.push("ca_vtime=" + (new Date().getTime() - ca_itime) + "-" + ca_visitor + "-" + ca_session);
        query.push("ca_preref=" + escape(ca_preref));
        query.push("ca_action=" + escape(act == null ? "0-0-PAGE:NORMAL" : ("0-0-PAGE:NORMAL(" + act + ")")));
    ...
    this._traceEvent = function (target, type, x, y, parentNum) {
        ...
        for (var i = 0; i < tags.length; i++) {
            ...
            if (target.attributes.getNamedItem(tag.name) != null && ((tag.name == "class" && this._hasClass(target, tag.value)) || (tag.name != "class" && this.matchTarget(target.attributes.getNamedItem(tag.name).nodeValue, tag.value)))) {
                ...
                query.push("ca_dm=" + ca_domain);
                query.push("ca_tenant=" + ca_tenantID);
                query.push("ca_ptype=" + (tag.ptype == "" ? ca_ptype : tag.ptype));
                query.push("ca_vtime=" + (cutime - ca_itime) + "-" + ca_visitor + "-" + ca_session);
                query.push("ca_preref=" + escape(ca_preref));
                query.push("ca_action=" + escape(x + "-" + y + "-PAGE:EVENT(" + (tag.action == "" ? (target.tagName + " " + tag.value) : tag.action) + ")"));
                query.push("ca_cookie=" + escape(document.cookie));
                query.push("ca_sessionid=" + _getCookie("ca_sessionid"));
                query.push("ca_uid=" + _getCookie("ca_uid"));
                query.push("ca_join=" + _getCookie("join_key"));
    ...
    this._traceClick = function (ca_domain, ca_tenantID, action) {
        ...
        query.push("ca_vtime=" + (cutime - ca_itime) + "-" + ca_visitor + "-" + ca_session);
        query.push("ca_preref=" + escape(ca_preref));
        query.push("ca_action=" + escape("0-0-PAGE:EVENT(" + action + ")"));
        query.push("ca_cookie=" + escape(document.cookie));
        query.push("ca_sessionid=" + _getCookie("ca_sessionid"));
        query.push("ca_uid=" + _getCookie("ca_uid"));
        query.push("ca_join=" + _getCookie("join_key"));
    ...
    this._traceConvert = function (ca_track_domain, ca_track_tenant, sbtType, act) {
        ...
        query.push("ca_cookie=" + escape(document.cookie));
        query.push("ca_action=" + escape(act == null ? "0-0-PAGE:CONVERT" : ("0-0-PAGE:CONVERT(" + act + ")")));
        query.push("ca_sessionid=" + _getCookie("ca_sessionid"));
        query.push("ca_uid=" + _getCookie("ca_uid"));
        query.push("ca_join=" + _getCookie("join_key"));
        this.last_query = query;
    ...
    this._traceTrans = function (act) {
        ...
        query.push("ca_vtime=" + (new Date().getTime() - ca_itime) + "-" + ca_visitor + "-" + ca_session);
        query.push("ca_preref=" + escape(ca_preref));
        query.push("ca_action=" + escape(act == null ? "0-0-PAGE:TRANS" : ("0-0-PAGE:TRANS(" + act + ")")));
        query.push("ca_cookie=" + escape(document.cookie));
        this.last_query = query;
    ...
    if (a === "get") {
        ...
        if (a === "post") {
            ...
            if (navigator.userAgent.indexOf("MSIE") > 0) {
                _traker.ca_frame = document.createElement("<iframe name='ca_postiframe'>");
                _traker.ca_form = document.createElement("<form name='ca_portform'>");
                _traker.ca_input = document.createElement("<input name='ca_transcontent'>");
...
function _addJoinKey() {
    ...
    var a = _getCookie("ca_uid") + "_" + _getCookie("ca_sessionid") + "_" + b;
    window.reseveKey = "#params:userId," + a;
    _addCookie("_DOYOO_RESEVE_KEY", window.reseveKey, 0, _parentDm);
    _addCookie("join_key", a, 36 * 30 * 24, _parentDm)
}
function _onAbort(b, a, c) {
    c.push("ca_abort=true");
    _traker.ca_img2.src = a + "?" + c
}
function _doGet(a, c) {
    try {
        _checkDOM("get");
        ...
        _traker.ca_img.src = a + "?" + c;
        _traker.last_query = null
    } catch (b) {
        _onAbort(_traker, a, c)
    }
}
function _doPost(a, c) {
    try {
        _checkDOM("post");
        _traker.ca_visitor = 0;
        _traker.ca_session = 0;
        _traker.ca_form.action = a;
        _traker.ca_input.value = c;
